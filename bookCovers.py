...
def buildCoverPages2(w, h, year):
    ...
    coverTitleStyle = dict(font='Upgrade-Thin', fontSize=100, textFill=c1, rTracking=0.2, rLeading=0.9, openTypeFeatures=dict(tnum=True) )
    bs = context.newString('9460\n7304\n7258\n0800\n512', style=coverTitleStyle, w=w-M/2)  
    bx, by, bw, bh = bs.bounds()
    context.text(bs, (w/2-(bw+bx)/2, -by+1.5*M))
...
IMAGES = (
    ('docs/documents/bookCoverPages1.pdf', W, H, now().year, buildCoverPages1),    ('docs/images/bookCoverPages1.png', W, H, now().year, buildCoverPages1),    ('docs/documents/bookCoverPages2.pdf', W, H, now().year, buildCoverPages2),    ('docs/images/bookCoverPages2.png', W, H, now().year, buildCoverPages2),)        
